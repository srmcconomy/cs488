...
bool Mesh::intersect(const vec3& eye, const vec3& ray, const mat4& transform, vec3& point, vec3& normal, float& d) {
  ...
  eye4 = inv * eye4;
  ray4 = inv * ray4;
  std::cout << to_string(eye4) << std::endl;
  ...
  normal = normalize(vec3(normal4.x, normal4.y, normal4.z));
  d = length(point - eye);
  return ret;
}

bool Mesh::intersect(const vec3& eye, const vec3& ray, vec3& point, vec3& normal, float& d) {
  std::cout << "eye " << to_string(eye) << std::endl;
  std::cout << "ray " << to_string(ray) << std::endl;
  bool isect = false;
